...
const cancelKey = (type: TaskType, userId: string) => [`task_cancel:${type}:${userId}`];
const activeKey = (type: TaskType, userId: string) => [`task_active:${type}:${userId}`];
...
const clampPercentage = (value: number) => Math.max(0, Math.min(100, Math.floor(value)));
...
const buildState = (status: ITaskState["status"], partial: Partial<ITaskState> = {}): ITaskState => ({
    status,
...
const getTaskState = async (type: TaskType, userId: string): Promise<ITaskState> => {
    ...
    if (current.value) {
        ...
        return {
            ...current.value,
            percentage: Number.isFinite(current.value.percentage) ? current.value.percentage : 0,
    ...
    if (legacy.value) {
        return buildState(legacy.value as ITaskState["status"], { startedAt: null });
    }
...
const sdevTasks = {
    forceStop: async (entry: QueueEntry) => {
        ...
        await tasks.set(statusKey(entry.type, entry.userId), buildState("stopped", { percentage: 0, startedAt: null }));
    ...
    enqueue: async (entry: QueueEntry) => {
        ...
        await tasks.set(statusKey(entry.type, entry.userId), buildState("queued", { percentage: 0, startedAt: null }));
    ...
    list: async (id: string): Promise<{ type: TaskType; name: string; state: ITaskState }[]> => {
        const taskTypes = Object.values(TaskType).filter((entry) => typeof entry === "number") as TaskType[];
        const rows = [];
...
tasks.listenQueue(async (entry: QueueEntry) => {
    ...
    await tasks.set(
        statusKey(entry.type, entry.userId),
        buildState("running", { percentage: 0, startedAt: runningStartedAt, error: null }),
    );
    let lastProgress = 0;
    ...
    try {
        switch (entry.type) {
            ...
            case TaskType.GenerateHeatmap:
                await heatmap.generate(entry.userId, strava, reportProgress, throwIfCancelled);
                break;
            ...
            case TaskType.GenerateActivityImages:
                await activityImages.generate(entry.userId, strava, reportProgress, throwIfCancelled);
                break;
            case TaskType.GenerateRouteImages:
                await routeImages.generate(entry.userId, strava, reportProgress, throwIfCancelled);
                break;
            default:
        ...
        if (error instanceof TaskCancelledError || (error instanceof Error && error.name === "TaskCancelledError")) {
            await tasks.set(
                statusKey(entry.type, entry.userId),
                buildState("stopped", { percentage: lastProgress, startedAt: null, error: null }),
            );
            return;
        }
        await tasks.set(
