        ...
        if (Number.isInteger(taskType) && taskType in TaskType) {
            if (action === "force_stop") {
                ...
                await sdevTasks.enqueue({
                    userId: folder,
                    type: taskType as TaskType,
                    body: bodyByType[taskType as TaskType] ?? "Task requested from Tasks page.",
                });
            }
        }
            ...
            <div class="data-health-page">
                <section>
                    ...
                    <table>
                        ...
                        <tbody id="task-rows">
                            {props.data.tasks.map((task) => <tr>
                                <td>{task.name}</td>
                                <td>{task.state.status}</td>
                                <td>{task.state.updatedAt || "-"}</td>
                                <td>{runningSince(task.state.startedAt)}</td>
                                <td>{task.state.error || "-"}</td>
                                <td>
                                    <div class="task-progress" role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={clamp(task.state.percentage)}>
                                        <div class="task-progress-fill" style={`width: ${clamp(task.state.percentage)}%;`} />
                                        <span class="task-progress-label">{clamp(task.state.percentage)}%</span>
                                    </div>
                                </td>
                                <td>
                                    {task.state.status === "running" && <form
                                        method="post"
                                        {...({ onsubmit: "return confirm('Force stop this task? This will request cancellation immediately.');" } as any)}
                                    >
                                        <input type="hidden" name="task_type" value={task.type.toString()} />
                                    ...
                                    {task.state.status !== "running" && <form method="post">
                                        <input type="hidden" name="task_type" value={task.type.toString()} />
                                        <input type="hidden" name="action" value="run" />
                                        <button type="submit" disabled={task.state.status === "queued"}>
                                            Run
                                        </button>
                                    </form>}
                                </td>
