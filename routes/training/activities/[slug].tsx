...
const buildDistanceSeries = (
  rawPoints: Omit<TrackPoint, "distanceKm" | "paceSecondsPerKm">[],
): TrackPoint[] => {
...
const withPaceSeries = (points: TrackPoint[]): TrackPoint[] => {
  ...
  for (let i = 0; i < paced.length; i++) {
    ...
    if (values.length > 0) {
      paced[i].paceSecondsPerKm = values.reduce((sum, value) =>
...
const parseGpxTrackPoints = async (path: string): Promise<TrackPoint[]> => {
  ...
  for (const node of trkpts as any[]) {
    ...
    const heartRate = toNullableNumber(
      findDescendantText(node, ["hr", "heartrate"]),
    );
    const cadence = toNullableNumber(
      findDescendantText(node, ["cad", "cadence"]),
    );
...
const interpolateAtDistance = (
  ...
  const ratio = Math.min(
    1,
    Math.max(0, (targetDistance - a.distanceKm) / segment),
  );

  const timestampMs = a.timestampMs != null && b.timestampMs != null
...
const buildRealSplits = (points: TrackPoint[]): Split[] => {
  ...
  const firstTime =
    points.find((point) => point.timestampMs != null)?.timestampMs ?? null;
...
const buildHrCadenceOverlaySvg = (points: TrackPoint[]): string | null => {
  const timed = points.filter((point) =>
    point.timestampMs != null
  ) as (TrackPoint & { timestampMs: number })[];
  ...
  const hr = timed.filter((point) =>
    point.heartRate != null
  ) as (TrackPoint & { timestampMs: number; heartRate: number })[];
  const cad = timed.filter((point) =>
    point.cadence != null
  ) as (TrackPoint & { timestampMs: number; cadence: number })[];
  ...
  const cadMin = cad.length
    ? Math.min(...cad.map((point) => point.cadence))
    : 0;
  const cadMax = cad.length
    ? Math.max(...cad.map((point) => point.cadence))
    : 1;
...
<svg viewBox="0 0 ${width} ${height}" class="metric-chart-svg" role="img" aria-label="Heart rate and cadence over time">
  ...
  ${
    hr.length >= 2
      ? `<polyline fill="none" stroke="#d1495b" stroke-width="2" points="${hrLine}" />`
      : ""
  }
  ${
    cad.length >= 2
      ? `<polyline fill="none" stroke="#2f81c7" stroke-width="2" points="${cadLine}" />`
      : ""
  }
  <text x="${chartX + 4}" y="${
    chartY + 14
  }" fill="#d1495b" font-size="11">HR</text>
  <text x="${chartX + 30}" y="${
    chartY + 14
  }" fill="#2f81c7" font-size="11">Cadence</text>
  <text x="${chartX}" y="${
    height - 8
  }" fill="#565666" font-size="11">Time</text>
</svg>`;
};
...
const compactTrackPoints = (points: TrackPoint[], maxPoints = 20000) => {
  const source = points.length > maxPoints
...
export const handler: Handlers<Props> = {
  async GET(_req: Request, ctx: FreshContext) {
    ...
    const interactionPoints = compactTrackPoints(
      trackPoints.length > 0 ? trackPoints : buildDistanceSeries(
    ...
    return ctx.render({
      ...
      hasCombinedSeries: interactionPoints.filter((point) =>
        point.elevation != null || point.paceSecondsPerKm != null
      ).length > 1,
    });
  },
};
...
export const Activity = ({ data }: PageProps<Props>) => {
  const movingSeconds = asNumber(
    data.activity.moving_time || data.activity.elapsed_time,
  );
    ...
    <>
      ...
      <details open>
        ...
        <section class="header">
          <TimeAgo value={data.activity.activity_date} />
      ...
      <section class="map">
        ...
        <div class="pace-interactive" id="pace-chart-wrap">
          <svg
            id="pace-chart-svg"
            class="pace-chart-svg"
            viewBox="0 0 900 110"
            preserveAspectRatio="none"
            role="img"
            aria-label="Pace profile"
          >
            <rect
              x="18"
              y="12"
        ...
        <div
          id="elevation-chart-empty"
          class="metric-card"
          style={data.hasCombinedSeries ? "display:none" : "display:block"}
        >
          <h3>Elevation / Pace</h3>
          <p>
            No per-point elevation or pace data found in this activity file.
          </p>
        </div>
        ...
        <article class="metric-card">
          ...
          <div class="unit-row">
            <UnitDisplay
              ...
              value={data.activity.distance}
            />
            <UnitDisplay
              unit=""
              description="Moving Time"
              value={secondsToClock(movingSeconds)}
            />
            <UnitDisplay
              unit="/km"
              description="Average Pace"
              value={pace.replace(" /km", "")}
            />
            <UnitDisplay
              unit="m"
              description="Elevation Gain"
              value={data.activity.elevation_gain}
            />
          </div>
          <p>
            <strong>Max speed:</strong> {data.activity.max_speed} km/h
          </p>
          <p>
            <strong>Average speed:</strong> {data.activity.average_speed} km/h
          </p>
          <p>
            <strong>Calories:</strong> {data.activity.calories}
          </p>
        </article>
      ...
      <section class="activity-detail-grid">
        <article class="metric-card">
          ...
          {data.hrCadenceOverlaySvg && (
            <div
              dangerouslySetInnerHTML={{ __html: data.hrCadenceOverlaySvg }}
            />
          )}
          {!data.hrCadenceOverlaySvg && (
            <p>Heart rate or cadence time-series not present in this file.</p>
          )}
          <p>
            HR avg/max: {data.activity.average_heart_rate || "-"} /{" "}
            {data.activity.max_heart_rate2 || data.activity.max_heart_rate ||
              "-"} bpm
            <br />
            Cadence avg/max: {data.activity.average_cadence || "-"} /{" "}
            {data.activity.max_cadence || "-"} rpm
          </p>
        </article>
      </section>
      ...
      <section class="activity-detail-grid">
        <article class="metric-card">
          ...
          {data.nearbyRoutes.length === 0 && (
            <p>No nearby routes detected from route geometry.</p>
          )}
        </article>
      </section>
    </>
